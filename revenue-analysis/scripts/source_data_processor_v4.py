...
# === Step 0: File Paths ===
SOURCE_FILE = "/mnt/data/v2 Rev Perf Report with Second Group Layer & CPT.xlsx"
...
df.loc[~df["Group_EM"].isin({"Existing E/M Code", "New E/M Code"}), "Avg. Charge E/M Weight"] = np.nan

# === Step 5: Build CPT List + Benchmark Keys ===
df["Charge CPT Code"] = df["Charge CPT Code"].astype(str).str.strip()
cpt_list_df = (
    df.groupby("Invoice_Number")["Charge CPT Code"]
